...
import st from '../../pages/TagsNewPage.module.scss'
      ...
      <div flex p-8px gap-x-16px overflow-auto text="#999">
        {emojis.map(emoji =>
          <span whitespace-nowrap key={emoji.name}
            className={emoji.name === emojiKind ? st.selectedTag : ''}
      ...
      <div text-24px p-t-8px p-b-16px h-300px overflow-auto>
        {emojis.map(emoji =>
          <div key={emoji.name} style={{ display: emoji.name === emojiKind ? '' : 'none' }}
            ...
            justify-center>
            {emoji.chars.map((char, index) =>
              <span b-1 b-transparent key={index} className={char === value ? s.selected : ''}
              rounded-4px onClick={() => value !== char && onChange?.(char)}>{char}</span>
            )}
          </div>
        )}
      </div>
